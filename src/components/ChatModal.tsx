      ...
      {isOpen && (
        <div className="fixed inset-0 z-[9999] flex items-end sm:items-center justify-center pointer-events-none">
          ...
          <motion.div 
            ...
            className="relative w-full max-w-lg bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl flex flex-col h-[90vh] sm:h-[650px] overflow-hidden m-0 sm:m-4 pointer-events-auto"
          >
            {/* Header */}
            <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-white shrink-0">
              ...
              >
                <div className="w-10 h-10 rounded-full bg-primary-50 flex items-center justify-center text-primary-600 font-bold border border-primary-100 uppercase">
                  {listing.agent?.name?.charAt(0) || 'A'}
                </div>
                <div>
                  <h3 className="font-bold text-slate-900 text-sm leading-none mb-1 truncate max-w-[150px] sm:max-w-none">{listing.agent?.name}</h3>
                  <div className="flex items-center gap-1">
                    <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Active Chat</p>
                  </div>
                </div>
              </button>
              <button 
            ...
            <div className="px-4 py-2 bg-slate-50 border-b border-slate-100 flex items-center gap-3 shrink-0">
              <img 
                src={listing.image} 
              ...
              <div className="flex-1 min-w-0 text-left">
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tight truncate">Inquiry for</p>
                    ...
                    <AnimatePresence mode="popLayout">
                      {(convStatus === 'inquiry' || convStatus === 'negotiating') && (
                        <motion.button 
                          ...
                          className="flex items-center gap-2 px-4 py-2 bg-primary-50 text-primary-700 rounded-xl text-xs font-bold whitespace-nowrap hover:bg-primary-100 transition-colors border border-primary-100 shadow-sm"
                        >
                      ...
                      {convStatus === 'contract_sent' && (
                        <motion.button 
                          ...
                          className="flex items-center gap-2 px-4 py-2 bg-emerald-50 text-emerald-700 rounded-xl text-xs font-bold whitespace-nowrap hover:bg-emerald-100 transition-colors border border-emerald-100 shadow-sm"
                        >
                    ...
                    <AnimatePresence mode="popLayout">
                      {convStatus === 'contract_requested' && (
                        <motion.button 
                          ...
                          className="flex items-center gap-2 px-4 py-2 bg-primary-600 text-white rounded-xl text-xs font-bold whitespace-nowrap hover:bg-primary-700 transition-colors shadow-lg shadow-primary-500/20"
                        >
                      ...
                      {convStatus === 'paid' && (
                        <motion.button 
                          ...
                          className="flex items-center gap-2 px-4 py-2 bg-emerald-600 text-white rounded-xl text-xs font-bold whitespace-nowrap hover:bg-emerald-700 transition-colors shadow-lg shadow-emerald-500/20"
                        >
            ...
            <div className="flex-1 overflow-y-auto p-4 bg-slate-50/30 space-y-4">
              ...
              {isLoading ? (
                ...
                <div className="h-full flex flex-col items-center justify-center text-center p-8">
                  <div className="w-16 h-16 bg-white rounded-2xl shadow-sm flex items-center justify-center mb-4 border border-slate-100 text-primary-200">
                ...
                messages.map((msg) => (
                  <div 
                    key={msg.id}
                    className={`flex ${msg.type === 'action' ? 'justify-center my-6' : msg.senderId === currentUser.id ? 'justify-end' : 'justify-start'}`}
                  >
                    {msg.type === 'action' ? (
                      <div className="bg-white border border-slate-200 py-3 px-5 rounded-2xl shadow-sm flex flex-col items-center text-center max-w-[80%]">
                        <div className="w-10 h-10 rounded-full bg-primary-50 flex items-center justify-center mb-3 text-primary-600">
                          {msg.actionType === 'paid' ? <CreditCard className="w-5 h-5" /> : <FileText className="w-5 h-5" />}
                        </div>
                        <p className="text-xs font-bold text-slate-800 mb-1 leading-tight uppercase tracking-wide">Transaction Step</p>
            ...
            {/* Input Area */}
            <form onSubmit={handleSendMessage} className="p-4 border-t border-slate-100 bg-white shrink-0 shadow-[0_-10px_20px_rgba(0,0,0,0.02)]">
              <div className="relative group flex items-center gap-2">
                ...
                <div className="relative flex-1">
                  <input 
                    ...
                    className="w-full bg-slate-100/80 border-2 border-transparent px-4 py-3.5 pr-10 rounded-2xl text-sm outline-none focus:bg-white focus:border-primary-500/20 focus:ring-4 focus:ring-primary-500/5 transition-all placeholder:text-slate-400"
                    disabled={isSending}
                  />
                  <button type="button" className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-warning-500 transition-colors">
                  ...
                  <AnimatePresence mode="wait">
                    {newMessage.trim() === '' && !isSending ? (
                      <motion.button 
                        ...
                        type="button"
                        className="w-11 h-11 rounded-xl flex items-center justify-center text-slate-400 hover:text-primary-600 hover:bg-primary-50 transition-all"
                      >
