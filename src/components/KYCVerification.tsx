...
import { ShieldCheck, Camera, FileText, CheckCircle2, Loader2, AlertCircle, Upload, ChevronRight, Fingerprint, Clock, CreditCard, Plane } from 'lucide-react';
...
const KYCVerification = () => {
  ...
  if (!user || (user.role !== 'agent' && user.role !== 'tenant')) return null;
      ...
      >
        ...
        <div className="flex-1 text-left">
          <p className="text-sm font-bold text-slate-900 dark:text-slate-100 flex items-center gap-2">
            {user.role === 'agent' ? 'Agent Verification' : 'Tenant Verification'}
            {hasRejection && <span className="text-[9px] bg-rose-100 dark:bg-rose-900/30 text-rose-600 dark:text-rose-400 font-bold uppercase tracking-widest px-2 py-0.5 rounded-full">Update Needed</span>}
          </p>
          <p className="text-[10px] text-slate-500 dark:text-slate-400 font-medium tracking-tight mt-0.5">
            {isVerified ? (user.role === 'agent' ? 'Verified Professional' : 'Verified Identity') : isPending ? 'Reviewing your credentials' : hasRejection ? 'Submission was declined' : 'Secure your account & build trust'}
          </p>
        </div>
        <div className={`px-2.5 py-1 rounded-full text-[9px] font-black uppercase border transition-colors flex items-center gap-1 ${
            isVerified ? 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 border-emerald-200 dark:border-emerald-800' :
            isPending ? 'bg-amber-50 dark:bg-amber-900/20 text-amber-600 dark:text-amber-400 border-amber-200 dark:border-amber-800' :
            hasRejection ? 'bg-rose-50 dark:bg-rose-900/20 text-rose-600 dark:text-rose-400 border-rose-200 dark:border-rose-800' :
            'bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-700'
        }`}>
            {isVerified ? <CheckCircle2 className="w-3 h-3" /> : isPending ? <Clock className="w-3 h-3" /> : hasRejection ? <AlertCircle className="w-3 h-3" /> : null}
              ...
              {!isVerified && !isPending && (
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    {idTypes.map((type) => {
                      let Icon = FileText;
                      if (type === 'Drivers License' || type === 'National ID Card') Icon = CreditCard;
                      if (type === 'Passport') Icon = Plane;
                      return (
                        <button
                          key={type}
                          disabled={isLoading}
                          ...
                          className={`p-3 rounded-2xl flex flex-col items-center justify-center gap-2 border-2 transition-all ${
                            idType === type 
                              ? 'bg-primary-50/50 dark:bg-primary-900/10 border-primary-500 text-primary-600 dark:text-primary-400 scale-[1.02] shadow-sm' 
                              : 'bg-white dark:bg-slate-800/50 border-slate-100 dark:border-slate-800 text-slate-600 dark:text-slate-400 hover:border-slate-300 dark:hover:border-slate-700'
                          }`}
                        >
                          <Icon className={`w-5 h-5 ${idType === type ? 'text-primary-500' : 'text-slate-400 dark:text-slate-500'}`} />
                          <span className="text-[9px] font-bold uppercase tracking-tighter text-center leading-tight">{type}</span>
                        </button>
                      );
                    })}
                  </div>
