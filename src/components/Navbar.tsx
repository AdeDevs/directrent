...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-[100] bg-white/70 backdrop-blur-xl border-b border-white/20 shadow-sm">
          ...
          <div className="hidden md:flex items-center space-x-6">
            ...
            <div className="flex items-center gap-3">
              <button 
                ...
                className="text-sm font-medium text-slate-600 hover:text-primary-600 transition-colors px-3 py-2"
              >
                Sign In
              </button>
              <button 
                onClick={() => { setAuthMode('signup'); setView('auth'); }}
                className="text-sm font-medium bg-primary-600 text-white px-5 py-2 rounded-xl hover:bg-primary-700 shadow-md transition-all shadow-primary-200"
              >
                Sign Up
              </button>
            </div>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div 
            ...
            className="md:hidden bg-white/80 backdrop-blur-2xl border-b border-slate-100 overflow-hidden shadow-xl shadow-slate-200/20"
          >
            <div className="px-4 pt-2 pb-6 flex flex-col space-y-4 shadow-inner shadow-slate-100/50">
              <a 
                href="#listings" 
              ...
              <div className="pt-2 flex flex-col gap-3">
                ...
                <button 
                  ...
                  className="w-full bg-primary-600 text-white px-4 py-3 rounded-xl font-semibold text-sm shadow-md shadow-primary-500/20 hover:bg-primary-700 transition-colors"
                >
                  Sign Up
                </button>
              </div>
